...
| `deleted` / `stopped` | 1. 恢复运行时备份2. 重新启动沙箱3. 调用 `/agent/start` |
...
| `provider`     | string  | 否  | 模型提供商：`openai`、`anthropic`、`google`、`ollama`、`azure`、`deepseek`、`glm`、`minimax`、`kimi`、`custom` |
...
| `MODEL_UPDATE_FAILED` | 500      | 模型配置更新失败 |
...
- `AGENT_NOT_FOUND`：Agent 不存在，检查 agent\_id 是否正确
- `SESSION_NOT_FOUND`：会话不存在（两边都查不到），检查 session\_id 是否正确
...
- local\_process 创建 Agent 返回 500：优先检查 `WITTY_AGENT_SERVER_APP_DIR` 是否正确，以及子进程里的 `witty-agent-server` 是否能正常响应 `/ping`
